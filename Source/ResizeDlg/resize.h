...
public:
	...
	int operator = (const REALRECT& rrc);
	int operator += (const REALRECT& rrc);
...
#define SZ_RESIZE_ITEM(x)	    CRealRect(0,0,x,x) // resize 
#define SZ_REPOS_ITEM(x)		CRealRect(x,x,x,x) // reposition 
#define SZ_HORRESIZE_ITEM(x)	CRealRect(0,0,x,0) // horizontal resize
#define SZ_HORREPOS_ITEM(x)	    CRealRect(x,0,x,0) // horizontal reposition
#define SZ_VERRESIZE_ITEM(x)	CRealRect(0,0,0,x) // vertical resize
#define SZ_VERREPOS_ITEM(x)	    CRealRect(0,x,0,x) // vertical reposition
