...
void CResize::Init()
{
	...
	if (!HasFlag(SZ_NOSIZEICON))
	{
		...
		SetResize(IDC_SIZEICON, SZ_REPOS_ITEM(1));
	}
}
...
BOOL CResizePropertySheet::OnInitDialog() 
{
	...
	// set sheet sizings
	SetResize(IDOK, SZ_REPOS_ITEM(1));
	SetResize(IDCANCEL, SZ_REPOS_ITEM(1));
	SetResize(IDHELP, SZ_REPOS_ITEM(1));
	SetResize(ID_APPLY_NOW, SZ_REPOS_ITEM(1));
	SetResize(AFX_IDC_TAB_CONTROL, SZ_RESIZE_ITEM(1));
	...
	{
		SetResize(GetPage(i), SZ_RESIZE_ITEM(1), rcPage);
	}
	return TRUE;
}
