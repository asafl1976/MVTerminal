...
C2DPushGraph::C2DPushGraph()
{
	...
	//SetLabelForMax((LPCTSTR)"100%");
	//SetLabelForMin((LPCTSTR)"0%");
...
bool C2DPushGraph::Push( int nMagnitude, UINT uiLineID )
{
	...
	CMagnitude m(nMagnitude,pLine->crLine);
	pLine->aMagnitudes.push_back(m);
	
	return true;
}

bool C2DPushGraph::Push( int nMagnitude, UINT uiLineID, COLORREF color )
{
	PushGraphLine *pLine = internal_LineFromID(uiLineID);
...
void C2DPushGraph::Update()
{
	...
	for (int n=0; n < m_aLines.GetSize(); ++n)
	{
		...
		//}
		if (nGreatest < (int)m_aLines[n]->aMagnitudes.size())
		{
			nGreatest = m_aLines[n]->aMagnitudes.size();
		}
	}
...
void C2DPushGraph::internal_DrawLines( CDC &dc, CRect& rect)
{
	...
	for (n = 0; n < m_aLines.GetSize(); ++n)
	{
		...
		if (!pLine->aMagnitudes.size())
		{
			CMagnitude m(m_nMinPeek, RGB( rand()%255 ,rand()%255 ,rand()%255));
			pLine->aMagnitudes.push_back(m);
		}
		...
		//}
		while ((int)pLine->aMagnitudes.size() < nGreatest)
		{	
			CMagnitude m( pLine->aMagnitudes[pLine->aMagnitudes.size()-1].m_nValue , RGB(rand()%255,rand()%255,rand()%255));
			pLine->aMagnitudes.push_back(m);
		}
		...
		//}
		while ((int)m_aLines[n]->aMagnitudes.size() >= m_nMaxCoords)
		{
			m_aLines[n]->aMagnitudes.erase(m_aLines[n]->aMagnitudes.begin());
		}

		//if (!m_aLines[n]->aMagnitudes.GetSize())
		if (!m_aLines[n]->aMagnitudes.size())
		{
		...
		penLine.CreatePen(PS_SOLID, m_nLineWidth , pLine->aMagnitudes[0].m_col);
		pMainPen = dc.SelectObject(&penLine);
		...
		{
			//dc.MoveTo(rect.left, nGreatest == 1 ? rect.Height() : rect.Height()-(pLine->aMagnitudes[0] * rect.Height()/div));	
			dc.MoveTo(rect.left, nGreatest == 1 ? rect.Height() : rect.Height()-(pLine->aMagnitudes[0].m_nValue * rect.Height()/div));	
		}
		penLine.DeleteObject();  

		
		//for (int n2 = 0; n2 < pLine->aMagnitudes.GetSize(); ++n2)
		for (int n2 = 0; n2 < (int)pLine->aMagnitudes.size(); ++n2)
		{
			penLine.CreatePen(PS_SOLID, m_nLineWidth, pLine->aMagnitudes[n2].m_col);
			...
			if (pLine->bShowAsBar)
			{
				...
				CRect rectBar;
				int nMag = pLine->aMagnitudes[n2].m_nValue;
				rectBar.left   = rect.left + (n2*(m_usLineInterval)) + 1;
				rectBar.right  = rectBar.left + GetInterval() - 1;
				rectBar.bottom = rect.Height();
				//rectBar.top    = rect.Height() - (pLine->aMagnitudes[n2] * rect.Height() / div);				
				rectBar.top    = rect.Height() - (nMag * rect.Height() / div);				
			...
			else
			{
				int nMag = pLine->aMagnitudes[n2].m_nValue;
				/* Draw a line */
				dc.LineTo(rect.left + (n2*m_usLineInterval), rect.Height() - (nMag * rect.Height() / (div)) );
				//dc.LineTo(rect.left + (n2*m_usLineInterval), rect.Height() - (pLine->aMagnitudes[n2] * rect.Height() / (div)) );


				//dc.FillSolidRect(rect.left + (n2*m_usLineInterval) - 2, 
				//	rect.Height() - (pLine->aMagnitudes[n2] * rect.Height() / (m_nMaxPeek-m_nMinPeek)) - 2, 4, 4, m_aLines[n]->crLine); 

			}
			penLine.DeleteObject(); // Free Pen 
		}
		dc.SelectObject(pMainPen);	
	}
...
void C2DPushGraph::internal_DrawGrid( CDC &dc, CRect& rect)
{
	...
	for (n = rect.Height()-1; n >= 0; n -= m_nGridSize)
	{
