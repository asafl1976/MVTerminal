...
BOOL CIPMMessageManager::HandleMessage(CIPMMessage  *pMsg)
{
	...
	case IPM_MVT_MARVELL_DEVICE_READ_REGISTER:
		{
			BOOL bRes = MarvellDeviceHwModuleReadRegister(pMsg);
			if(bRes == FALSE)
			{
				AfxMessageBox("Failed to process message \"Marvell Device - Read Register\"!");
			}
			return bRes;
		}
	case IPM_MVT_MARVELL_DEVICE_WRITE_REGISTER:
		{
			BOOL bRes = MarvellDeviceHwModuleWriteRegister(pMsg);
			if(bRes == FALSE)
			{
				AfxMessageBox("Failed to process message \"Marvell Device - Write Register\"!");
			}
			return bRes;
		}
