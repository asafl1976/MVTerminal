											   ...
											   CString strCurSelecetion /*= ""*/, int iSel /*= -1*/)
{
	...
	//TODO:  Send message to dlg to check if ok to open combo
	if(::SendMessage(m_pParentDlg->m_hWnd,WM_GPIO_BASIC_DLG_ON_COMBO_START,(WPARAM)iRowIndex,(WPARAM)iColumnIndex) == 0)
		return NULL;
	...
	pInPlaceCombo->SetParentDlg(m_pParentDlg);
...
CInPlaceEdit* CComboListCtrl::ShowInPlaceEdit(int iRowIndex, int iColumnIndex, CString& rstrCurSelection)
{
	...
	pInPlaceEdit->SetParentDlg(m_pParentDlg);
